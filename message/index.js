...
module.exports = msgHandler = async (bocchi = new Client(), message) => {
    try {
        ...
        switch (command) {
            ...
            arg = body.trim().split('|')
            if (arg.length >= 3) {
            bocchi.reply(from, 'Processing...', message.id) 
            const quotes = arg[1]
            const author = arg[2]
            const theme = arg[3]
            try {
            const result = await quotemaker(quotes, author, theme)
            bocchi.sendFile(from, result, 'quotesmaker.jpg','neh...')
            } catch {
            bocchi.reply(from, 'I\'m afraid to tell you that the image failed to process', message.id)
            }
            } else {
            bocchi.reply(from, 'Usage: \n!quotemaker |text|watermark|theme\n\nEx :\n!quotemaker |...|...|random', message.id)
            }
            break
                case prefix+'ytmp3':
                if (!isRegistered) return await bocchi.reply(from, eng.notRegistered(), id)
          ...
          //  break
            ...
            case prefix+'limit':
                ...
                await bocchi.reply(from, `⤞ Limit left: ${limit.getLimit(sender.id, _limit, limitCount)} / 10\n\n*_Limit Will Reset at 00:00 Server Time_*`, id)
            break
